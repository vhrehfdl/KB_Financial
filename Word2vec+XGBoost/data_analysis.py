...
def label_distribution_table(train, test):
    ...
    genre_num = le.fit_transform(train["category"])
    print(genre_num)
...
def text_length_analysis(data):
    train_length = data["content"].astype(str).apply(len)
...
def wordcloud(data):
    cloud = WordCloud(width=800, height=600).generate(" ".join(data["content"].astype(str)))
